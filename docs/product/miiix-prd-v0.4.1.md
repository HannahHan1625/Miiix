...
- [x] 主发布提交 `d9d292a` 已推送至 GitHub `main`。
...
- 已完成 30 个黄金样本。
- 在菜谱关系合同和数据库迁移演练完成后，再决定 200 个高频食材的扩充顺序。
- 建立别名、分类、储存、单位、营养和图片授权。
- 建立 JSON Schema、导入脚本和数据质量测试。

### v0.4.2.1 数据源接入与形态模型

唯一主目标：让 30 条黄金数据具备可重放来源链，并把“食材是什么”与“现在是什么形态”分开。

- 固定外部来源修订、许可、用途、checksum、importer 和审核批次；应用运行时不联网抓源数据。
- 引入 `concept / variant / form_projection / inventory lot` 分层，以及受控 form/process 字段。
- “猪肉末”是“猪肉”的 `ground` 形态投影；买来就是肉糜时保留 purchased 批次事实。
- v0.4.1 技术 ID `pork` 仅在 namespaced migration 中指向猪肉末；新英文 `pork` 指向广义猪肉。
- Epicure 本版只做固定词表映射，不导入菜谱正文、共现边或推荐服务。
- 用户加工库存必须等待原子转换用例，不能由写采购流水的通用 `createLot` 冒充。

明确不包含：200 项扩库、菜谱库、搭配排序、AI 生图、OCR/VLM、Supabase 和 UI 重做。

已记录但不在本补丁实现的图片决策：生成菜谱的成品图不抓取第三方菜谱图片，后续接 AI 生图；统一构图、光线、餐具和色彩风格要在 UX 设计阶段试验后确认，并保存模型、prompt 与人工主体审核记录。

### 下一数据门槛：菜谱共现与搭配关系

- 明确菜谱来源、抓取/复制/再发布权利与 attribution。
- 定义 recipe identity、ingredient occurrence、形态、用量、步骤和关系版本。
- 建立合理性、惊喜度、三样食材覆盖率和额外采购量的离线评测。
- 缺失或 pending 关系必须有显式回退，不能默认退化成无意义的水煮菜。
...
- [`v0.4.2.1 下一数据会话 Prompt`](../handoff/v0.4.2.1-session-prompt.md)
- [`v0.4.2.1 数据源接入与形态模型`](../data/v0.4.2.1-source-and-form-model.md)
- [`版本总账`](../../outputs/version-history.md)
- [`数据库说明`](../../database/README.md)
- [`Repository 说明`](../../src/data/repositories/README.md)

本文档基线状态：**v0.4.2.1 只处理可重放数据源和食材形态；菜谱共现/搭配质量、部署和真实 AI 必须分别定版，不得混入本补丁。**
